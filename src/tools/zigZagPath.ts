...
const getZigZagAxisCoordinate = (triangleHeight: number, triangleBase: number, coordinate: any, { shift = 0 }) => {
  return (
    Math.abs((coordinate + shift) / triangleBase - Math.floor((coordinate + shift) / triangleBase) - 0.5) *
    triangleHeight *
    2
  );
};
...
const getZigZagPointList = (
  ...
  switch (axis) {
    case Axis.horizontal: {
      // ending ??= rect.bottom;
      const startX = getZigZagAxisCoordinate(triangleHeight, triangleBase, 0, { shift: shift });
      let startY = 0;
    ...
    case Axis.vertical:
      ...
      {
        let startX = 0;
        const startY = getZigZagAxisCoordinate(triangleHeight, triangleBase, 0, { shift: shift });
        pointList.push(point(startX + offset.dx, startY + offset.dy));
...
export const getZigZagPath = (
  ...
  ctx.beginPath();
  const pointList = getZigZagPointList(triangleHeight, triangleBase, rect, axis, {
    offset: offset,
    ending: ending,
    shift: shift
  });
...
export const getZigZagRectPath = (
  triangleHeight: number,
  triangleBase: number,
  rect: any,
  axis: string,
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  { offset, ending, shift = 0, clipStart = true, clipEnd = true, stroke = 1 }: any,
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  ctx: any
) => {
  ctx.lineWidth = stroke;
  const topPointList = getZigZagPointList(triangleHeight, triangleBase, rect, axis, {
    offset: offset,
    ending: ending,
    shift: shift
  });
  const bottomPointList = getZigZagPointList(triangleHeight, triangleBase, rect, axis, {
    offset: offset,
    ending: ending,
